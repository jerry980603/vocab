...
2. **用真實資料校正 `Q_PER_UNIT` / `SEC_PER_UNIT`**（目前是 5.4 題／76 秒，
...
- **錯題優先清最近答錯的**：`buildQueue("wrong")` 目前是隨機抽。
  剛錯的字記憶還新，答對一次就畢業，比從三週前的舊帳開始清有效率。
- **錯題段分開顯示「今天新增 X 個／舊帳 Y 個」**，讓使用者看得出有沒有在還債。
...
6. **快篩按「不會」會讓今天的進度爆量**：`renderScreenCard()` 的
