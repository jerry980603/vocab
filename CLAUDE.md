...
**下一批要做**：**第 3 級 N–Z 的可考字，約 130 個**（`w45`）。
2026/09/05 實際把第 3 級待補清單掃過一遍，發現舊版這份文件寫得太悲觀——
那批不是全部都冷僻，A–M 就挑得出 133 個學測會考的字（已做成 `w44`）。
N–Z 同樣還有一批值得收，例如：

> navy、neighborhood、novelist、passenger、passport、pearl、pilot、plastic、
> politician、portrait、producer、professor、racial、receipt、republic、
> robbery、rocket、romantic、scholar、scientist、sexual、silk、statue、
> suicide、survivor、sword、tablet、tank、tent、ton、tourist、tower、trunk、
> twin、underlying、verse、web、wed、wrist、youngster

挑完 N–Z 之後，1～5 級**有價值的部分就真的做滿了**，剩下的才是
kangaroo／spaghetti／lollipop／zipper 那種純冷僻具體名詞。

| 分級 | 還缺 | 性質 |
|---|---|---|
| 第 3 級 | 253 字 | N–Z 還有約 130 個可考字（見上），其餘是冷僻具體名詞 |
| 第 4 級 | 78 字 | 全是冷僻具體名詞（aquarium、broom、lobster、pasta、raisin 這類） |
| 第 5 級 | 1 字 | `ass`，刻意不收（見下面） |
| 第 1 級 | 4 字 | `mister`／`nope`／`ok`／`yeah`，刻意不收（口語變體） |

目標範圍是 1～5 級的話，字庫已經覆蓋 4755／5091 ＝ **93%**。
...
情境線索不足，現在**全部改寫完畢**。全庫 11463 句例句，短於 10 字的 **0 句**，
平均 11.28 個英文字，每個資料檔都通過 `check.py`。
...
| 第 1 級 | 4 字 | `mister`／`nope`／`ok`／`yeah` | **刻意不收**，口語變體 |
| 第 2 級 | 0 | — | 完結 |
| 第 3 級 | 253 字 | N–Z 還有約 130 個可考字，其餘是冷僻具體名詞 | **值得再做一批**（`w45`） |
| 第 4 級 | 78 字 | 同上（aquarium、broom、lobster…） | 同上 |
| 第 5 級 | 1 字 | `ass` | **刻意不收**（只剩粗俗義） |
| 第 6 級 | 1023 字 | 目標範圍外 | 除非使用者把課表的目標範圍切成「全部 6 級」 |

**目標範圍（1～5 級）的覆蓋率是 4755／5091 ＝ 93%，內容已經不是瓶頸了。**
學習期 100 天 × 每天 70 個新字義 ＝ 7000，字庫有 7804 個字義，**整個計畫都夠用**。
再做完第 3 級 N–Z 的可考字（`w45`）之後，就可以停止補字了。
...
| `data/w4.js` | 單字 N–R |
| `data/w5.js` | 單字 S–Z |
| `data/w6.js`～`data/w44.js` | 補充批次。`w6`～`w13` 前半第 3 級，`w13`～`w19` 第 4 級，`w20`～`w27` 第 5 級，`w28`～`w34` 第 2 級，`w35`～`w43` 第 1 級，`w44` 回頭補第 3 級 |
| `data/p1.js` | 片語 |
...
目前進度：官方 6114 個字裡，**已編好釋義與例句的有 4755 個**（以上面的「字庫進度」為準）。
