...
**舊例句改寫進度（2026/09 開始）**：`w1`～`w5` 與 `p1` 原本有 987 句短於 10 字，
情境線索不足。目前已改寫 **357 句**，還剩 **630 句**：
`p1.js` 96、`w3.js` 162、`w4.js` 206、`w5.js` 166。
（`w1.js` 170 句、`w2.js` 187 句已完成，兩個檔現在都 ✅ 全部通過。）

改寫方式：用 `check.py` 列出短句 → 逐句重寫成 10～14 字並重寫中譯 →
寫成 `(舊英文, 新整行)` 的對照表，用腳本比對英文部分整行替換，
腳本會檢查每一句都確實命中、沒有漏掉也沒有重複比對。
一次處理一個檔比較好驗收。
